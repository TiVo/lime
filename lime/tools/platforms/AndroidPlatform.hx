...
class AndroidPlatform extends PlatformTarget {
	...
	public override function build ():Void {
		...
		for (architecture in architectures) {
			...
			var haxeParams = [ hxml, "-D", "android" ];
			var cppParams = [ "-Dandroid" ];
	...
	public override function rebuild ():Void {
		...
		if (armv5) commands.push ([ "-Dandroid"]);
		if (armv7) commands.push ([ "-Dandroid", "-DHXCPP_ARMV7", "-DHXCPP_ARM7"]);
		if (x86) commands.push ([ "-Dandroid", "-DHXCPP_X86"]);
